...
researchers <- researchers |> 
  ...
  mutate(grupo = as.integer(grupo)) |> 
   left_join(grupos |> select(grupo, nombre_grupo, universidad),
                     by = c("grupo")) |> 
  ...
  mutate(grupo = paste0(grupo,
         ...
         nombre_grupo = paste0(nombre_grupo,
                               collapse = "; "),
         universidad = paste0(universidad,
                              collapse = "; ")) |> 
  unique()
```
...
grafo <- graph_from_data_frame(d = edges, 
                      directed = FALSE, 
                      vertices = nodes) 
grafo |> 
...
## Modularidad
grafo_modularidad <- cluster_louvain(grafo)

grafo_prueba$community <- grafo_modularidad$membership

unique(grafo_prueba$community)

## agrupacion comunidades
data <- data.frame()

for (i in unique(grafo_prueba$community)) {
  
  induced_subgraph(grafo_prueba, v = which(grafo_prueba$community == i))
...
knitr::kable(
  data |> 
    dplyr::select(data, n_characters, most_important)
)
```
...
nodes <- 
  ...
  mutate(color = if_else(universidad == "otra", "blue", "red"))
